...
- Patron de infraestructura productiva registrado: cuando el Droplet ya tenga Nginx host compartido para otros servicios, el despliegue del sistema debe ejecutar solo rontend y ackend en Docker con puertos enlazados a loopback (127.0.0.1) y delegar terminacion TLS, vhosts y enrutamiento de dominios al Nginx del host para evitar conflictos en 80/443.
- Patron backend de conectividad registrado: para PostgreSQL gestionada con sslmode=require y cadena no confiable por defecto en runtime Node, la configuracion inicial debe permitir DATABASE_SSL=true con DATABASE_SSL_REJECT_UNAUTHORIZED=false hasta incorporar CA confiable explicita, evitando reinicios por self-signed certificate in certificate chain.
