...
- Patron de infraestructura/CD registrado: la autenticacion a GHCR en GitHub Actions debe resolver `username/password` como par consistente segun el token usado; con `GHCR_WRITE_TOKEN` usar cuenta estable (`GHCR_USERNAME` o owner), y sin PAT usar `github.actor` junto a `GITHUB_TOKEN`.
