...
- Patron backend/frontend registrado: en la accion publica de cliente `Llamar mesero`, la UI debe permitir capturar un mensaje opcional y enviarlo en `notes` dentro de `createPublicServiceRequest`, manteniendo la solicitud valida incluso cuando el mensaje venga vacio.
- Patron backend/frontend registrado: la creacion de pedidos publicos del cliente debe validarse en backend contra una `CashSession` abierta del restaurante actual; si la caja esta cerrada, el endpoint debe rechazar la operacion con error de negocio explicito.
- Patron frontend registrado: el estado operativo de caja debe exponerse de forma persistente en la navegacion lateral administrativa mediante un badge visible cuando no exista caja abierta, consumiendo `cash-session/current` con `react-query` y claves `['admin', 'cash-session', ...]`.
- Patron frontend registrado: cuando una vista administrativa exponga codigos QR operativos, debe ofrecer accion de impresion desde la misma tarjeta y generar un layout imprimible con identidad del restaurante (nombre y contacto disponible) mas datos de contexto del recurso (por ejemplo mesa, zona y capacidad), evitando depender de capturas manuales o estilos de pantalla.

- Patron de documentacion registrado: ademas del README tecnico y MANUAL de uso, el repositorio debe mantener GUIA_FLUJO_SISTEMA.md con el flujo E2E actualizado y GUIA_MODULOS_VALIDACIONES.md con validaciones DTO, negocio y UI por modulo, marcando como No determinado lo no verificable en codigo.
- Patron frontend registrado: en flujos de impresion iniciados por accion del usuario, la ventana de impresion debe abrirse de forma sincrona al click y luego poblarse de manera asincrona (por ejemplo tras generar QR), para evitar bloqueos de popup por perdida de gesto de usuario.
- Patron backend/frontend registrado: el QR fisico de mesa debe permanecer estable; la seguridad operacional se resuelve rotando/inutilizando `sessionToken` por ciclo de atencion, cerrando automaticamente la `DiningSession` al saldo cero, bloqueando escrituras publicas sobre sesiones pagadas/cerradas y permitiendo que `public/start` trate `existingSessionToken` como hint opcional con fallback automatico a sesion activa vigente o nueva sesion de la mesa.
- Patron de infraestructura/CD registrado: el despliegue productivo a Droplet debe ejecutarse con GitHub Actions en dos fases (CI de frontend y backend + CD), construyendo imagenes en GHCR por commit, desplegando con deploy/docker-compose.prod.yml, generando configuracion Nginx por dominio en el servidor y publicando solo cuando lint, tests y build de ambos modulos hayan pasado.
- Patron de infraestructura/CD registrado: la autenticacion a GHCR en GitHub Actions debe resolver `username/password` como par consistente segun el token usado; si existe `GHCR_WRITE_TOKEN` se debe exigir `GHCR_USERNAME` explicito, y sin PAT usar `github.actor` junto a `GITHUB_TOKEN`.
- Patron de infraestructura/CD registrado: los healthchecks del paso de deploy en Droplet deben tolerar warm-up de contenedores con reintentos y espera incremental, y al agotar intentos deben imprimir logs del servicio fallido para diagnostico rapido.
- Patron backend de infraestructura registrado: cuando la base de datos sea PostgreSQL gestionada externa (por ejemplo DigitalOcean Managed DB), la conexion TypeORM debe habilitar SSL configurable por entorno con DATABASE_SSL y DATABASE_SSL_REJECT_UNAUTHORIZED, manteniendo compatibilidad local con SSL desactivado cuando corresponda.
- Patron de integracion continua registrado: la validacion de calidad debe ejecutarse en pull request hacia main con jobs separados de frontend y backend, incluir smoke tests de despliegue minimo, y proteger la rama principal exigiendo esos checks antes de merge.
- Patron frontend registrado: en flujos de impresion que escriben HTML en una ventana emergente (por ejemplo QR de mesas), la ventana debe abrirse por gesto del usuario sin `noopener/noreferrer` para conservar el handle, y `print()` debe ejecutarse cuando el documento este cargado (`readyState` completo o evento `load`) para evitar impresiones vacias o popup no controlable.
- Patron backend de infraestructura registrado: en imagenes Docker de backend NestJS, el comando de arranque productivo del `Dockerfile` debe estar alineado con `start:prod` de `package.json` y con la salida real de compilacion TypeScript (`dist/src/main.js` en la configuracion actual), evitando rutas hardcodeadas divergentes como `dist/main.js`.
